...
class CheckPaymentEndpoints(object):
    async def CheckPaymentStatus(self, **kwargs):
        ...
        body = {
            "type": self.transactionType.get('checkPaymentPTP').get('type'),
            "processingCode": self.transactionType.get('checkPaymentPTP').get('code'),
            "amount": amount,
            "date": timestampStr,
        ...
        req = await httpx.post(self.ovo_url, json=body, headers=header)
        try:
            return req.json()
        except ValueError:
